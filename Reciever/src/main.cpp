...
String SEALEVELPRESSURE_HPA = "";     // sea level pressure in hPa, for BMP388 altitude calculation
...
void loop()
{
  ...
  if (SA_COMMAND == true && SA_STATUS == false && millis() - lastSendTime > interval) {
    sendPacket("SA" + SEALEVELPRESSURE_HPA);
    lastSendTime = millis();            // timestamp the message
    interval = random(1000) + 500;    // 0.5-1.5 seconds
    Serial.print("[*] SA Command Sent with Pressure: ");
  ...
  while (Serial.available() > 0)
  {
    ...
    else
    {
      ...
      if (message[0] == 'S' && message[1] == 'A')
      {
        ...
        SA_COMMAND = true;
        String hpa_string = "";
        // Update the sea level pressure from four characters after SA
        for (int i = 2; i < 6; i++)
        {
          hpa_string += message[i];
        }
        SEALEVELPRESSURE_HPA = hpa_string;
      }
